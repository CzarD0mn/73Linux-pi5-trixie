...
chmod +x "$DEST/bin/trixie-apt.sh" "$DEST/73.sh"

# Overlay any patched files present in this repo
for rel in 73.sh changelog bin/set-enviroment.sh \
  app/stable/pi/HAMLIB.bapp app/stable/pi/CHIRP.bapp \
  app/stable/pi/FLDIGI.bapp app/stable/pi/FLRIG.bapp \
  app/stable/pi/EES.bapp app/stable/pi/PITERM.bapp \
  app/stable/pi/QTSOUND.bapp app/stable/pi/HAMRS.bapp; do
  if [ -f "$HERE/$rel" ]; then
...
if ! grep -q 'pi5-trixie-fork' "$DEST/73.sh"; then
